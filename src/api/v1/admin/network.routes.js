// network.routes.js - 网络配置管理路由（PostgreSQL适配版）
...
// 获取所有网络配置
router.get('/networks', adminAuth, hasRole(['super_admin', 'finance_admin']), async (req, res) => {
    try {
        let networks = [];
...
// 获取单个网络配置
router.get('/networks/:network', adminAuth, hasRole(['super_admin', 'finance_admin']), async (req, res) => {
    try {
        let network = null;
...
router.post('/networks/:network', adminAuth, hasRole(['super_admin', 'finance_admin']), async (req, res) => {
    ...
    try {
        ...
        if (isProduction) {
            const result = await query('SELECT * FROM network_config WHERE network = $1', [network]);
            oldConfig = result?.[0];
        } else {
            const db = getDb();
            oldConfig = db.prepare('SELECT * FROM network_config WHERE network = ?').get(network);
        }
        
        let exists = false;
        if (isProduction) {
            const result = await query('SELECT id FROM network_config WHERE network = $1', [network]);
            exists = result && result.length > 0;
        } else {
            const db = getDb();
            const result = db.prepare('SELECT id FROM network_config WHERE network = ?').get(network);
            exists = !!result;
        }
...
// 获取单个网络地址（用户端）
router.get('/user/addresses/:network', async (req, res) => {
    const { network } = req.params;
